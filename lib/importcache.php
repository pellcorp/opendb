...
function import_cache_get_cache_directory() {
	...
	if (is_dir ( $cacheDir )) {
		...
		opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Import Cache directory does not exist', array (
				$cacheDir ) );
		return FALSE;
	}
}
...
function import_cache_insert($user_id, $plugin_name, $infile_location) {
	if (file_exists ( $infile_location )) {
		...
		if ($content_length > 0) {
			...
			if (is_dir ( $directory )) {
				...
				if ($insert && db_affected_rows () > 0) {
					...
					if (copy ( $infile_location, $directory . '/' . $cache_file ) !== FALSE) {
						...
						opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Error copying files', array (
								$user_id,
								$plugin_name,
								$infile_location,
				...
				opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Importcache directory is not accessible', array (
						$user_id,
						$plugin_name,
						$infile_location,
						$directory ) );
			...
			opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Input file is empty', array (
					$user_id,
					$plugin_name,
					$infile_location ) );
			return FALSE;
		...
		opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Input file not found', array (
				$user_id,
				$plugin_name,
				$infile_location ) );
		return FALSE;
...
function import_cache_delete($sequence_number) {
	...
	if (is_array ( $cache_r )) {
		...
		if ($delete && db_affected_rows () > 0) {
			opendb_logger ( OPENDB_LOG_INFO, __FILE__, __FUNCTION__, 'Deleted import_cache record', array (
					$sequence_number ) );
