		...
		if (is_user_granted_permission(PERM_ADMIN_USER_LISTING)) {
			...
			if (is_numeric($listingObject->getItemsPerPage())) {
				...
				if ($listingObject->getTotalItemCount() > 0) {
					$result = fetch_user_rs(NULL, //$user_role_permissions
					INCLUDE_ROLE_PERMISSIONS, INCLUDE_CURRENT_USER, $restrict_users_mode, $listingObject->getCurrentOrderBy(), $listingObject->getCurrentSortOrder(), $listingObject->getStartIndex(), $listingObject->getItemsPerPage());
				}
			} else {
				$result = fetch_user_rs(NULL, //$user_role_permissions
				INCLUDE_ROLE_PERMISSIONS, INCLUDE_CURRENT_USER, $restrict_users_mode, $listingObject->getCurrentOrderBy(), $listingObject->getCurrentSortOrder()); //$include_deactivated_users
			}
				...
				while ($user_r = db_fetch_assoc($result)) {
					...
					if ($HTTP_VARS['restrict_active_ind'] != 'X') {
						if ($user_r['active_ind'] == 'Y') {
							$listingObject->addColumn((strlen($user_r['lastvisit']) > 0 ? get_localised_timestamp(get_opendb_config_var('user_admin', 'datetime_mask'), $user_r['lastvisit']) : get_opendb_lang_var('never_logged_in')));
						} else {
			...
			if ($HTTP_VARS['restrict_active_ind'] != 'X') {
				echo ("<li>" . getToggleControl($PHP_SELF, $HTTP_VARS, get_opendb_lang_var('show_deactivated_users'), 'show_deactivated_users', ifempty($HTTP_VARS['show_deactivated_users'], 'N')) . "</li>");
			}
			...
			opendb_not_authorised_page(PERM_ADMIN_USER_LISTING, $HTTP_VARS);
		}
	} else {
