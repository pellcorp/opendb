	...
	while ($file = readdir($handle)) {
		if (!preg_match("/^\./", $file) && preg_match("/Upgrader_(.*).class.php$/", $file, $regs)) {
			...
			if ($upgraderPlugin !== NULL) {
				$upgrader_rs[] = array('to_version' => $upgraderPlugin->getToVersion(), 'from_version' => $upgraderPlugin->getFromVersion(), 'description' => $upgraderPlugin->getDescription(), 'upgrader_plugin' => $upgraderRef);
