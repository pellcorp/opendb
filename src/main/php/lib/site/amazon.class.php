...
class amazon extends SitePlugin {
				...
				} else if (preg_match("!<li><b>ISBN-10:</b>\s*([0-9]+)</li>!", $pageBuffer, $regs)) // for books, ASIN is the same as ISBN
 {
			...
			if ($amazonasin !== FALSE) {
				...
				if ((preg_match("/ id=\"resultCount\">.*?<span>Showing.[0-9]+[\s]*-[\s]*[0-9]+.of.([0-9,]+) Results<\//", $pageBuffer, $regs) || preg_match("/ id=\"resultCount\">.*?<span>Showing.([0-9]+).Result.*?<\//", $pageBuffer, $regs))) {
