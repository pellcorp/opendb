...
class pilot extends movie_base {
	...
	protected function set_pagename($wt) {
		switch ($wt) {
		case "Title":
			$urlname = "/movies/imdb-id-" . (int) $this->imdbid() . ".json";
			break;
		case "Credits":
			$urlname = "/movies/imdb-id-" . (int) $this->imdbid() . "/casts.json";
			break;
		case "Trailers":
			$urlname = "/movies/imdb-id-" . (int) $this->imdbid() . "/trailer.json";
			break;
		case "Images":
			$urlname = "/movies/imdb-id-" . (int) $this->imdbid() . "/images.json";
			break;
		default:
	...
	public function movieTypes() {
		if ($this->pilot_imdbfill)
			$this->main_movietypes = $this->imdb->movieTypes();
	...
	public function movietype() {
		if ($this->pilot_imdbfill)
			$this->main_movietype = $this->imdb->movieType();
	...
	public function runtimes() {
		if (empty($this->movieruntimes)) {
			...
			if (empty($country[0]) || empty($runtime))
				return array();
	...
	public function aspect_ratio() {
		if ($this->pilot_imdbfill)
			$this->aspectratio = $this->imdb->aspect_ratio();
	...
	public function language() {
		if ($this->pilot_imdbfill > BASIC_ACCESS)
			$this->main_language = $this->imdb->language();
	...
	public function languages() {
		if ($this->pilot_imdbfill > BASIC_ACCESS)
			$this->langs = $this->imdb->languages();
		return $this->langs;
	}
	...
	public function languages_detailed() {
		if ($this->pilot_imdbfill > BASIC_ACCESS)
			$this->lang_fulls = $this->imdb->languages_detailed();
	...
	public function colors() {
		if ($this->pilot_imdbfill > BASIC_ACCESS)
			$this->moviecolors = $this->imdb->colors();
	...
	public function creator() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->main_creator = $this->imdb->creator();
	...
	public function tagline() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->main_tagline = $this->imdb->tagline();
	...
	public function seasons() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->seasoncount = $this->imdb->seasons();
		else
			return 0;
	...
	public function is_serial() {
		if ($this->pilot_imdbfill > NO_ACCESS)
			return $this->imdb->is_serial();
	}
	...
	public function plotoutline($fallback = FALSE) {
		if ($this->main_plotoutline == "") {
			...
			if (empty($this->main_plotoutline) && $fallback)
	...
	private function thumbphoto() {
		...
		if ($this->page["Title"] == "cannot open page")
			return false;
		// no such page
		if (empty($this->page["Title"]->poster))
			return false;
		// no pics
		$baseurl = $this->page["Title"]->poster->base_url . $this->page["Title"]->poster->photo_id . "/" . $this->page["Title"]->poster->file_name_base;
	...
	public function mainPictures() {
		if (empty($this->main_pictures)) {
			...
			for ($i = 0; $i < $total_entries; ++$i) {
				$baseurl = $this->page['Images'][$i]->base_url . $this->page['Images'][$i]->photo_id . "/" . $this->page['Images'][$i]->file_name_base;
	...
	public function mpaa_hist() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->mpaas_hist = $this->imdb->mpaa_hist();
	...
	public function mpaa_reason() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->mpaa_justification = $this->imdb->mpaa_reason();
	...
	public function prodNotes() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->main_prodnotes = $this->imdb->prodNotes();
	...
	public function top250() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->main_top250 = $this->imdb->top250();
	...
	public function plot_split() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->split_plot = $this->imdb->plot_split();
	...
	public function synopsis() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->synopsis_wiki = $this->imdb->synopsis();
	...
	public function taglines() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->taglines = $this->imdb->taglines();
	...
	private function castlist() {
		if (empty($this->castlist)) {
			if (empty($this->page["Credits"]))
				$this->openpage("Credits");
			foreach (array("actor", "soundtrack", "director", "screenplay", "production") as $function)
				$this->castlist[$function] = array(); // fill the important ones
			if ($this->invalidPage("Credits"))
				return $this->castlist;
			// no such page
			//$this->page["Credits"]->{'total_entries'} equals count($this->page["Credits"]->{'movies_people'})
			if (is_array($this->page["Credits"]->{'movies_people'}))
	...
	public function crazy_credits() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->crazy_credits = $this->imdb->crazy_credits();
	...
	public function episodes() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->season_episodes = $this->imdb->episodes();
	...
	public function quotes() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->moviequotes = $this->imdb->quotes();
	...
	public function trailers($full = FALSE) {
		if (empty($this->trailers)) {
			if (empty($this->page["Trailers"]))
				$this->openpage("Trailers");
			if ($this->invalidPage("Trailers"))
	...
	public function soundtrack() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->soundtracks = $this->imdb->soundtrack();
	...
	public function movieconnection() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->movieconnections = $this->imdb->movieconnection();
	...
	public function extReviews() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->extreviews = $this->imdb->extReviews();
	...
	public function prodCompany() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->compcred_prod = $this->imdb->prodCompany();
	...
	public function distCompany() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->compcred_dist = $this->imdb->distCompany();
	...
	public function specialCompany() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->compcred_special = $this->imdb->specialCompany();
	...
	public function otherCompany() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->compcred_other = $this->imdb->otherCompany();
	...
	public function parentalGuide() {
		if ($this->pilot_imdbfill == FULL_ACCESS)
			$this->parental_guide = $this->imdb->parentalGuide();
	...
	public function officialSites() {
		if (empty($this->official_sites)) {
			...
			if (!empty($this->page["Title"]->alternative_identifiers->amazon))
				$this->official_sites[] = array("url" => "http://www.amazon.com/dp/" . $this->page["Title"]->alternative_identifiers->amazon . "/", "name" => "Amazon");
		}
