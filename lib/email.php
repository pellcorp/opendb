...
function get_email_footer() {
	if (strlen ( get_opendb_lang_var ( 'email_footer' ) ) > 0) {
		...
		$footer_text = get_opendb_lang_var ( 'email_footer', array (
				'site' => get_opendb_config_var ( 'site', 'title' ),
				'version' => get_opendb_version (),
				'site_url' => $site_url ) );
...
function opendb_user_email($to_userid, $from_userid, $subject, $message, &$errors, $append_site_to_subject = TRUE) {
	...
	if (is_user_permitted_to_receive_email ( $to_userid )) {
		...
		if (sendEmail ( $to_email_addr, $to_name, $from_email_addr, $from_name, $subject, $message, $errors )) {
			...
			insert_email ( $to_userid, $from_userid != $from_email_addr ? $from_userid : NULL, 
				$from_email_addr, $subject, $message );
...
function sendEmail($to, $toname, $from, $fromname, $subject, $message, &$errors) {
	...
	if ($mailer->Send ()) {
		...
		opendb_logger ( OPENDB_LOG_INFO, __FILE__, __FUNCTION__, 'Email sent', array (
				$to,
				$toname,
				$from,
				$fromname,
		...
		// No errors returned indicates correct execution.
		opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, $mailer->ErrorInfo, array (
				$to,
				$toname,
				$from,
				$fromname,
...
function insert_email($to_user_id, $from_user_id, $from_email_addr, $subject, $message) {
	...
	if (! is_user_valid ( $to_user_id )) {
		...
		opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Invalid To User', array (
				$to_user_id,
				$from_user_id,
				$from_email_addr,
				$subject ) );
		...
		opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Invalid From User', array (
				$to_user_id,
				$from_user_id,
				$from_email_addr,
				$subject ) );
		...
		opendb_logger ( OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, 'Invalid From Email', array (
				$to_user_id,
				$from_user_id,
				$from_email_addr,
				$subject ) );
