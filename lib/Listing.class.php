...
class Listing
{
	...
	*/
	...
	function _addHelpEntry($help_text, $img=NULL, $id=NULL)
	{
		...
		{
			$this->_help_entry_rs[] = array(img=>$img, text=>$help_text, type=>$id);
	...
	function addHeaderColumn($title, $fieldname = NULL, $sortColumn = TRUE, $type = NULL)
	{
		...
		if($fieldname == 'title')
		{
			if(is_user_granted_permission(PERM_VIEW_ITEM_COVERS) && 
					...
					((get_opendb_config_var('listings', 'allow_override_show_item_image')===FALSE && get_opendb_config_var('listings', 'show_item_image')!==FALSE) ||
					get_opendb_config_var('listings', 'allow_override_show_item_image')!==FALSE && ((get_opendb_config_var('listings', 'show_item_image')!==FALSE && 
