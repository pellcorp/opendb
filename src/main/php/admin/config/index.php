...
function get_group_block($config_group_r) {
	...
	if ($results) {
		$config_group_rs = $OPENDB_CONFIGURATION->getGroup($config_group_r['id']);

		while ($config_group_item_r = db_fetch_assoc($results)) {
			if (strpos($config_group_item_r['type'], 'array') !== FALSE) {
				$values_r = $config_group_rs[$config_group_item_r['id']];
			} else {
				$values_r = $config_group_rs[$config_group_item_r['id']][$config_group_item_r['keyid']];
			}
