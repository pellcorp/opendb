...
function fetch_item_instance_rs($item_id, $owner_id = NULL) {
	...
	$query = "SELECT ii.item_id, ii.instance_no, ii.owner_id, ii.borrow_duration, ii.s_status_type, ii.status_comment, UNIX_TIMESTAMP(ii.update_on) AS update_on " . " FROM item_instance ii, s_status_type sst, user u, item i"
...
function fetch_category_item_cnt($category, $s_item_type = NULL) {
	$query = "SELECT COUNT(DISTINCT ii.item_id) AS count " . "FROM item i, item_instance ii, s_status_type sst, user u, s_attribute_type sat, s_item_attribute_type siat, item_attribute ia " . "WHERE u.user_id = ii.owner_id AND " . "siat.s_item_type = i.s_item_type AND "
			. "sat.s_attribute_type = siat.s_attribute_type AND " . "sat.s_field_type = 'CATEGORY' AND " . "ia.s_attribute_type = siat.s_attribute_type AND " . "ia.item_id = ii.item_id AND " . "(ia.instance_no = 0 OR ia.instance_no = ii.instance_no) AND " . "ia.order_no = siat.order_no AND "
			. "u.active_ind = 'Y' AND " . "sst.s_status_type = ii.s_status_type AND " . "i.id = ii.item_id AND " . "ia.lookup_attribute_val = '" . $category . "'";
...
function is_exists_title($title, $s_item_type, $owner_id = NULL) {
	$query = "SELECT 'x' FROM item i,item_instance ii, s_status_type sst, user u " . "WHERE i.id = ii.item_id AND " . "u.user_id = ii.owner_id AND " . "u.active_ind = 'Y' AND " . "sst.s_status_type = ii.s_status_type AND " . "i.title = '" . addslashes($title) . "' AND " . "i.s_item_type = '"
			. $s_item_type . "' ";
	...
	if (is_array($column_display_config_rs)) {
		for ($i = 0; $i < count($column_display_config_rs); $i++) {
			...
			if ($column_display_config_rs[$i]['column_type'] == 's_attribute_type') {
				...
				// and ITEMTYPE.
				$field_type_fieldnames_r = array('ITEM_ID' => 'i.id', 
				//'TITLE'=>'i.title',
		...
		for ($i = 0; $i < count($column_display_config_rs); $i++) {
			if ($column_display_config_rs[$i]['column_type'] == 's_attribute_type') {
				if ($column_display_config_rs[$i]['search_attribute_ind'] != 'y') {
					...
					if ($query_type != 'COUNT') {
						$left_join = 'LEFT JOIN item_attribute ia' . $i . ' ON ' . 'ia' . $i . '.item_id = i.id AND (ia' . $i . '.instance_no = 0 OR ia' . $i . '.instance_no = ii.instance_no) AND ia' . $i . '.s_attribute_type = \'' . $column_display_config_rs[$i]['s_attribute_type'] . '\' AND ia'
								. $i . '.attribute_no = 1';
