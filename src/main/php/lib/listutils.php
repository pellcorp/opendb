...
function get_list_username($user_id, $mode, $subject = NULL, $redirect_link = NULL, $redirect_url = NULL) {
	...
	if ($user_id == get_opendb_session_var('user_id')) {
		...
		if (is_user_granted_permission(PERM_VIEW_USER_PROFILE))
			return "<a href=\"user_profile.php?uid=" . $user_id . "&subject=" . urlencode(ifempty($subject, get_opendb_lang_var('no_subject'))) . "&redirect_link=" . urlencode($redirect_link) . "&redirect_url=" . urlencode($redirect_url) . "\" title=\""
					. htmlspecialchars(get_opendb_lang_var('user_profile')) . "\">$user_name</a>";
		else
			return $user_name;
	}
}
...
function getAlphaListBlock($PHP_SELF, $HTTP_VARS) {
	...
	if (get_opendb_config_var('listings', 'alphalist_new_search_context') !== FALSE) {
		$context_vars = array('owner_id' => $HTTP_VARS['owner_id'], 'order_by' => $HTTP_VARS['order_by'], 'sortorder' => $HTTP_VARS['sortorder']);
	}
...
function getItemsPerPageControl($PHP_SELF, $HTTP_VARS) {
	...
	if (is_not_empty_array($items_per_page_options_r)) {
		...
		$buffer .= "<form class=\"itemsPerPageControl\" id=\"form-items_per_page\" action=\"" . $PHP_SELF . "\" method=\"GET\">" . get_url_fields($HTTP_VARS) . "<label for=\"select-items_per_page\">" . get_opendb_lang_var('items_per_page') . '</label>'
				. "<select id=\"select-items_per_page\" name=\"items_per_page\" class=\"footer\" onChange=\"this.form.submit()\">"
				. custom_select('items_per_page', $items_per_page_rs, '%display%', 'NA', ifempty($HTTP_VARS['items_per_page'], get_opendb_config_var('listings', 'items_per_page')), 'value') . "\n</select></form>";
	}
	return $buffer;
}
...
function getToggleControl($PHP_SELF, $HTTP_VARS, $text, $fieldname, $value) {
	$buffer .= "<form class=\"toggleControl\" id=\"toggle-$fieldname\" action=\"" . $PHP_SELF . "\" method=\"GET\">" . get_url_fields($HTTP_VARS, NULL, array($fieldname)) . "<input type=\"hidden\" name=\"$fieldname\" value=\"$value\">" . "<label for=\"toggle-$fieldname-cbox\">" . $text
			. "</label><input type=\"checkbox\" class=\"checkbox\" id=\"toggle-$fieldname-cbox\" name=\"${fieldname}_cbox\" value=\"Y\" onclick=\"if(this.checked){this.form['${fieldname}'].value='Y';}else{this.form['${fieldname}'].value='N';} this.form.submit()\""
			. (strcasecmp($value, 'Y') === 0 ? " CHECKED" : "") . ">" . "</form>";
	return $buffer;
}
