...
function get_input_field($fieldname, $s_attribute_type, $prompt, $input_type, $compulsory_ind = 'N', $value = NULL, $dowrap = TRUE, $prompt_mask = NULL, $onchange_event = NULL, $disabled = FALSE) {
	...
	return get_item_input_field($fieldname,
			array( // $item_attribute_type_r
					's_attribute_type' => $s_attribute_type, 'order_no' => NULL, 'prompt' => $prompt, 'input_type' => $input_type_def['type'], 'input_type_arg1' => $input_type_def['args'][0], 'input_type_arg2' => $input_type_def['args'][1], 'input_type_arg3' => $input_type_def['args'][2],
					'input_type_arg4' => $input_type_def['args'][3], 'input_type_arg5' => $input_type_def['args'][4], 'compulsory_ind' => $compulsory_ind), NULL, // $item_r
...
function validate_input_field($prompt, $input_type, $compulsory_ind = 'N', $value, &$errors) {
	...
	return validate_item_input_field(
			array( // $item_attribute_type_r
					'prompt' => $prompt, 'input_type' => $input_type_def['type'], 'input_type_arg1' => $input_type_def['args'][0], 'input_type_arg2' => $input_type_def['args'][1], 'input_type_arg3' => $input_type_def['args'][2], 'input_type_arg4' => $input_type_def['args'][3],
					'input_type_arg5' => $input_type_def['args'][4], 'compulsory_ind' => $compulsory_ind), $value, $errors);
}
...
function filter_input_field($input_type, $value) {
	...
	return filter_item_input_field(
			array( // $item_attribute_type_r
					'input_type' => $input_type_def['type'], 'input_type_arg1' => $input_type_def['args'][0], 'input_type_arg2' => $input_type_def['args'][1], 'input_type_arg3' => $input_type_def['args'][2], 'input_type_arg4' => $input_type_def['args'][3],
		...
		if ($item_attribute_type_r['lookup_attribute_ind'] != 'Y' && strlen($value[$i]) > 0) {
			...
			case 'datetime':
				...
				if ($components !== FALSE) {
					...
					$value[$i] = str_pad($components['year'], 4, '0', STR_PAD_LEFT) . str_pad($components['month'], 2, '0', STR_PAD_LEFT) . str_pad($components['day'], 2, '0', STR_PAD_LEFT) . str_pad($components['hour'], 2, '0', STR_PAD_LEFT) . str_pad($components['minute'], 2, '0', STR_PAD_LEFT)
...
function datetime_field($name, $prompt, $format_mask, $auto_datetime, $compulsory_ind, $value, $onchange_event, $disabled = FALSE, $multi_value = FALSE) {
	if (get_opendb_config_var('widgets', 'enable_javascript_validation') !== FALSE) {
		$onchange = "onchange=\"" . compulsory_ind_check($prompt, $compulsory_ind) . "if(this.value.length > 0 && !is_datetime(this.value, '" . $format_mask . "')){alert('" . get_opendb_lang_var('datetime_is_not_valid', array('prompt' => $prompt, 'format_mask' => $format_mask))
				. "');return false;} $onchange_event return true;\"";
	} else {
		$onchange = "onchange=\"$onchange_event\"";
	}
...
function enhanced_checkbox_field($name, $prompt, $checked_value, $unchecked_value, $value, $onclick_event = NULL, $disabled = FALSE) {
	...
	return "\n<input type=\"hidden\" name=\"$name\" value=\"" . ($is_checked ? $checked_value : $unchecked_value) . "\">" . "\n\t<input type=\"checkbox\" class=\"checkbox\" name=\"" . $name
			. "_cbox\" onclick=\"if (this.checked){this.form['$name'].value='$checked_value';}else{this.form['$name'].value='$unchecked_value';}\" $onclick " . ($is_checked ? "CHECKED" : "") . "" . ($disabled ? ' DISABLED' : '') . ">";
}
...
function url($name, $item_r, $item_attribute_type_r, $prompt, $length, $maxlength, $content_groups, $value, $onchange_event, $disabled = FALSE, $multi_value = FALSE) {
	...
	if ($item_attribute_type_r['file_attribute_ind'] == 'Y') {
		...
		$field .= "<input type=\"button\" class=\"button\" onclick=\"if(this.form['$name'].value.length>0){popup(this.form['$name'].value,'400','300');}else{alert('" . get_opendb_lang_var('prompt_must_be_specified', 'prompt', $prompt) . "');}\" value=\"" . get_opendb_lang_var('view') . "\""
				. ($disabled ? ' DISABLED' : '') . ">";
		$field .= "</div>";
