...
function get_op_confirm_form($PHP_SELF, $confirm_message, $HTTP_VARS) {
	...
	$formContents .= "<p>" . $confirm_message . "</p>" . get_url_fields($HTTP_VARS, NULL, array('confirmed')) . // Pass all http variables
			"<fieldset>" . "<label for=\"confirm_yes\">" . get_opendb_lang_var('yes') . "</label>" . "<input type=\"radio\" class=\"radio\" name=\"confirmed\" id=\"confirm_yes\" value=\"true\">" . "<label for=\"confirm_no\">" . get_opendb_lang_var('no') . "</label>"
			. "<input type=\"radio\" class=\"radio\" name=\"confirmed\" id=\"confirm_no\" value=\"false\" CHECKED>" . "</fieldset>" . "<input type=\"submit\" class=\"submit\" value=\"" . get_opendb_lang_var('submit') . "\">" . "</form>\n";
...
function format_checkbox_action_links($cbname, $not_checked_message, $action_links_rs) {
	...
	while (list(, $action_links_r) = @each($action_links_rs)) {
		...
		if ($action_links_r['checked'] !== FALSE) {
			$field .= "<a href=\"#\" onclick=\"if(isChecked(document.forms['" . $cbname . "'], '" . $cbname . "[]')){doFormSubmit(document.forms['" . $cbname . "'], '" . $action_links_r['action'] . "', '" . $action_links_r['op'] . "');}else{alert('" . $not_checked_message . "');} return false;\">"
					. $action_links_r['link'] . "</a>";
		} else {
