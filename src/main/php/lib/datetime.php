...
function get_lang_var_months_r($abbrev = FALSE) {
	...
	return array(get_opendb_lang_var('january' . $suffix), get_opendb_lang_var('february' . $suffix), get_opendb_lang_var('march' . $suffix), get_opendb_lang_var('april' . $suffix), get_opendb_lang_var('may' . $suffix), get_opendb_lang_var('june' . $suffix), get_opendb_lang_var('july' . $suffix),
			get_opendb_lang_var('august' . $suffix), get_opendb_lang_var('september' . $suffix), get_opendb_lang_var('october' . $suffix), get_opendb_lang_var('november' . $suffix), get_opendb_lang_var('december' . $suffix));
}
...
function get_lang_var_days_r($abbrev = FALSE) {
	...
	return array(get_opendb_lang_var('sunday' . $suffix), get_opendb_lang_var('monday' . $suffix), get_opendb_lang_var('tuesday' . $suffix), get_opendb_lang_var('wednesday' . $suffix), get_opendb_lang_var('thursday' . $suffix), get_opendb_lang_var('friday' . $suffix),
...
function get_localised_timestamp($format_mask, $timestamp = NULL) {
	...
	$php_mask_conversion = array("DDth" => "jS", //Day of the month with English suffix (1st, 2nd, 3rd, etc.)
...
function get_timestamp_components_for_datetime($datetime, $format_mask) {
							...
							if (strlen($year) && (($year % 4 == 0 && $year % 100 != 0) || $year % 400 == 0)) // leap year
 {
				...
				return array(year => intval($year), month => intval($month), day => intval($day), hour => intval($hour), minute => intval($minute), second => intval($second));
			} else {
				return FALSE;
			}
		} else {
