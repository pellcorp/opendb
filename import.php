...
function get_row_column_mappings_table($s_item_type, $owner_id, $header_row, $field_column_r, $field_default_r, $field_initcap_r) {
	...
	$buffer .= "\n<tr>" . "<th class=\"prompt\" scope=\"row\">" . get_opendb_lang_var('s_item_type') . "</th>" . "<td class=\"data fieldColumn\">" . get_column_select_block("field_column[s_item_type]", $header_row, $field_column_r['s_item_type']) . "</td>"
			. "<td class=\"data defaultColumn\">&nbsp;</td>" . "<td class=\"data initcapColumn\">&nbsp;</td>" . "</tr>";
			...
			if ($item_attribute_type_r['s_field_type'] !== 'ITEM_ID') {
				...
				if ($item_attribute_type_r['s_field_type'] == 'STATUSTYPE') {
					...
					if ($lookup_results) {
						...
						$default_field = get_item_input_field("field_default[$fieldname]", $item_attribute_type_r, NULL, //$item_r
						$field_default_r[$fieldname], FALSE);
				...
				$buffer .= "\n<tr>" . "<th class=\"prompt\" scope=\"row\">" . $item_attribute_type_r['prompt'] . "</th>" . "<td class=\"data fieldColumn\">" . get_column_select_block("field_column[$fieldname]", $header_row, $field_column_r[$fieldname]) . "</td>"
						. "<td class=\"data defaultColumn\">" . $default_field . "</td>" . "<td class=\"data initcapColumn\">" . $initcap_field . "</td>" . "</tr>";
...
function get_uploaded_form(&$importPlugin, $header_row, $HTTP_VARS) {
	...
	if ($importPlugin->get_plugin_type() == 'row') {
		...
		if ($results && db_num_rows($results) > 1) {
			$buffer .= format_field(get_opendb_lang_var('s_status_type'), radio_grid('s_status_type', $results, '%img%', // mask
			'VERTICAL', $HTTP_VARS['s_status_type'])); // value
		...
		if (is_user_granted_permission(PERM_USER_IMPORT) || is_user_granted_permission(PERM_ADMIN_IMPORT)) {
			...
			if (is_file_upload_enabled()) {
				...
				if ($HTTP_VARS['op'] == 'upload') {
					...
					} else { //if(is_not_empty_array($import_cache_r))
