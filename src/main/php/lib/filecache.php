...
function fetch_url_file_cache_r($url, $cache_type = 'HTTP', $mode = EXCLUDE_EXPIRED) {
	...
	$query = "SELECT sequence_number, upload_file_ind, cache_type, url, location, cache_file, cache_file_thumb, content_type, content_length, UNIX_TIMESTAMP(cache_date) as cache_date, IF(expire_date IS NOT NULL, UNIX_TIMESTAMP(expire_date), NULL) AS expire_date " . "FROM file_cache "
...
function fetch_file_cache_rs($cache_type = 'HTTP', $order_by = NULL, $sortorder = NULL, $start_index = NULL, $items_per_page = NULL) {
	$query = "SELECT sequence_number, cache_type, url, location, upload_file_ind, cache_file, cache_file_thumb, content_type, content_length, UNIX_TIMESTAMP(cache_date) as cache_date, IF(expire_date IS NOT NULL, UNIX_TIMESTAMP(expire_date), NULL) AS expire_date, IF(expire_date<=NOW(),'Y','N') as expired_ind "
			. "FROM file_cache " . "WHERE cache_type = '$cache_type' ";
...
function fetch_file_cache_r($sequence_number) {
	if (is_numeric($sequence_number)) {
		$query = "SELECT sequence_number, url, upload_file_ind, location, content_type, cache_type, content_length, UNIX_TIMESTAMP(cache_date) as cache_date, IF(expire_date IS NOT NULL, UNIX_TIMESTAMP(expire_date), NULL) AS expire_date, cache_file, cache_file_thumb " . "FROM file_cache "
...
function update_file_cache($sequence_number, $content_length, $expire_date, $cache_file, $cache_file_thumb) {
	if (is_numeric($sequence_number)) {
		$query = "UPDATE file_cache " . "SET content_length = $content_length, " . "cache_date = NOW(), " . "expire_date = " . ($expire_date != NULL ? $expire_date : "NULL") . ", " . "cache_file = '$cache_file', " . "cache_file_thumb = "
				. ($cache_file_thumb != NULL ? "'$cache_file_thumb'" : "NULL") . " " . "WHERE sequence_number = $sequence_number ";
