...
class amazonecs extends SitePlugin {
	...
	function queryListing($page_no, $items_per_page, $offset, $s_item_type, $search_vars_r) {
		if (strlen($search_vars_r[$this->siteAttributeType]) > 0) {
			...
			$queryUrl = "http://webservices.amazon." . $siteDomain . "/onca/xml?Service=AWSECommerceService&AWSAccessKeyId=" . $this->getConfigValue('amazon_access_key', 0) . "&Operation=ItemSearch&SearchIndex=" . $index_type . "&ResponseGroup=Small,Images&ItemPage=" . $page_no . "&Keywords="
					. rawurlencode($search_vars_r['title']);
	...
	function amazonCharacterData($parser, $data) {
		...
		if ($this->inItem) {
			...
			// Todo need to manage Amazon Errors
				if ($parentTag == 'Error' && $data != "AWS.ECommerceService.NoExactMatches")
