...
function get_system_admin_tools_menu($admin_type = NULL) {
	$admin_menu_rs = array('config' => array('link' => 'Configuration'), 'logfile' => array('link' => 'Log File'), 'backup' => array('link' => 'Backup Database'), 'http_cache' => array('link' => 'HTTP Cache Admin'), 'item_cache' => array('link' => 'Item Cache Admin'),
			's_language' => array('link' => 'Language Configuration'), 's_file_type' => array('link' => 'Supported File Types'), 's_title_display_mask' => array('link' => 'Title Display Mask Configuration'), 's_item_listing_conf' => array('link' => 'Item Listing Configuration'),
			's_status_type' => array('link' => 'System Status Types'), 's_item_type_group' => array('link' => 'System Item Type Groups'), 's_item_type' => array('link' => 'System Item Types'), 's_attribute_type' => array('link' => 'System Attribute Types'),
			's_address_type' => array('link' => 'System Address Types'), 's_site_plugin' => array('link' => 'Site Plugins'), 'patch_facility' => array('link' => 'Miscellaneous Patches'), 'announcements' => array('link' => 'Announcements'), 's_role' => array('link' => 'Role Permissions'),);
	...
	if (is_not_empty_array($filelist)) {
		...
		if (is_not_empty_array($sqllist)) {
			...
			for ($i = 0; $i < count($sqllist); $i++) {
				echo ("<tr class=\"oddRow\">" . "<td>" . $sqllist[$i]['type'] . (strlen($sqllist[$i]['description']) > 0 ? " - " . $sqllist[$i]['description'] : "") . "</td>" . "<td>" . $sqllist[$i]['sqlfile'] . "</td>" . "<td><a href=\"admin.php?type=$ADMIN_TYPE&op=installsql&sqlfile="
						. $sqllist[$i]['sqlfile'] . "\">Install</a></td>" . "</tr>");
			}
			echo ("</table>");
		}
	}
