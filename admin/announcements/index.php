...
function get_edit_announcement_input_form($announcement_r, $HTTP_VARS = NULL) {
	...
	$buffer .= get_input_field("title", NULL, //s_attribute_type
			'Title', "text(50,500)", //input type
			"Y", //compulsory!
			ifempty($announcement_r['title'], $HTTP_VARS['title']), TRUE);

	$buffer .= get_input_field("content", NULL, //s_attribute_type
			'Announcement', "htmlarea(60,15)", //input type
			"Y", //compulsory!
			ifempty($announcement_r['content'], $HTTP_VARS['content']), TRUE);

	$buffer .= get_input_field("display_days", NULL, //s_attribute_type
			'Display Days', "number(10,10)", //input type
			"Y", //compulsory!
			ifempty($announcement_r['display_days'], $HTTP_VARS['display_days']), TRUE);

	if (is_array($announcement_r)) {
		$buffer .= get_input_field("closed_ind", NULL, //s_attribute_type
				'Closed', "checkbox(Y,N)", //input type
				"N", //compulsory!
				ifempty($announcement_r['closed_ind'], $HTTP_VARS['closed_ind']), TRUE);
	}
