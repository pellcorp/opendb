...
class imdb_trailers {
	...
	function getFlashCodeMovieMaze($url) {
		...
		for ($i = 0; $i < $mc; ++$i) {
			if (strpos($matches[1][$i], "http://") === 0)
				...
				$list[] = array("url" => $this->moviemazeurl . $matches[1][$i] . "." . $matches[2][$i], "format" => $matches[2][$i]);
		}
		return $list;
	}
	...
	function getFlashCodeAllTrailers($url) {
		...
		for ($i = 0; $i < $mc; ++$i) {
			if (strpos($matches[1][$i], "http://") === 0)
				$list[$i] = array("url" => $matches[1][$i], "format" => "flv");
			else
				$list[$i] = array("url" => $this->alltrailersurl . $matches[1][$i], "format" => "flv");
		}
		return $list;
	}
	...
	function getYoutubeTrailers($url) {
		...
		#some times array?
		if (is_array($video))
			return array("url" => $video[0], "format" => "flv");
		else
			return array("url" => $video, "format" => "flv");
	}
