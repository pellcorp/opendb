...
function fetch_user_address_attribute_val($ua_sequence_number, $s_attribute_type, $order_no = NULL) {
	...
	$query = "SELECT uaa.attribute_val " . "FROM user_address ua," . "user_address_attribute uaa " . "WHERE ua.sequence_number = '" . $ua_sequence_number . "' AND " . "ua.start_dt <= now() AND (ua.end_dt IS NULL OR ua.end_dt < now()) AND " . "uaa.ua_sequence_number = ua.sequence_number AND "
...
function fetch_user_address_lookup_attribute_val($ua_sequence_number, $s_attribute_type, $order_no = NULL) {
	// Only load previous record if edit.
	$query = "SELECT uaa.lookup_attribute_val " . "FROM user_address ua," . "user_address_attribute uaa " . "WHERE ua.sequence_number = '" . $ua_sequence_number . "' AND " . "ua.start_dt <= now() AND (ua.end_dt IS NULL OR ua.end_dt < now()) AND " . "uaa.ua_sequence_number = ua.sequence_number AND "
			. "uaa.s_attribute_type = '" . $s_attribute_type . "' ";
...
function fetch_address_type_attribute_type_rs($s_address_type, $mode = 'query', $order_by = FALSE) {
	$query = "SELECT sadt.s_address_type, " . "saatr.s_attribute_type, " . "saatr.order_no, " . "if(length(IFNULL(stlv.value,saatr.prompt))>0,IFNULL(stlv.value,saatr.prompt),IFNULL(stlv2.value,sat.prompt)) as prompt, " . "sat.display_type, " . "sat.display_type_arg1," . "sat.display_type_arg2,"
			. "sat.display_type_arg3," . "sat.display_type_arg4," . "sat.display_type_arg5," . "sat.input_type," . "sat.input_type_arg1," . "sat.input_type_arg2," . "sat.input_type_arg3," . "sat.input_type_arg4," . "sat.input_type_arg5," . "sat.listing_link_ind ";
...
function insert_user_address($user_id, $s_address_type, $public_address_ind, $borrow_address_ind, $start_dt = NULL, $end_dt = NULL) {
	if (strlen($user_id) > 0) {
		...
		$query = "INSERT INTO user_address (user_id, s_address_type, start_dt, end_dt, public_address_ind, borrow_address_ind)" . " VALUES ('" . $user_id . "','" . $s_address_type . "'," . (strlen($start_dt) > 0 ? $start_dt : "now()") . ", " . (strlen($end_dt) > 0 ? "'" . $end_dt . "'" : "NULL")
				. ", '$public_address_ind', '$borrow_address_ind')";
