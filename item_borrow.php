				...
				else if($HTTP_VARS['op'] == 'admin_quick_check_out')
				{
					...
					if(is_user_admin(get_opendb_session_var('user_id'),get_opendb_session_var('user_type')))
					{
						...
						else
						{
									...
									if(is_exists_item_instance($item_id, $instance_no))
									{
										if(!is_item_borrowed($item_id, $instance_no))
										{
											if(!is_user_owner_of_item($item_id, $instance_no, $HTTP_VARS['borrower_id']))
											{
												...
												{
													$admin_quick_check_out_error = get_opendb_lang_var('s_status_type_items_cannot_be_borrowed', 's_status_type_desc', $status_type_r['description']);
												}
											}
											else
											{
												$admin_quick_check_out_error = get_opendb_lang_var('user_is_owner_of_item');
											}
										}
										else
										{
											$admin_quick_check_out_error = get_opendb_lang_var('item_is_already_checked_out');
										}
									}
									else
									{
										$admin_quick_check_out_error = get_opendb_lang_var('item_not_found');
									}
								}
								else
								{
									$admin_quick_check_out_error = get_opendb_lang_var('item_is_already_selected');
								}
							}
							...
							{
								echo("<input type=button onclick=\"doFormSubmit(this.form, '$PHP_SELF', 'quick_check_out')\" value=\"".get_opendb_lang_var('check_out_item(s)')."\">");
