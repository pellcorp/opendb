...
class amazonfr extends SitePlugin {
	...
	function queryListing($page_no, $items_per_page, $offset, $s_item_type, $search_vars_r) {
		...
		if (strlen($pageBuffer) > 0) {
			...
			if ($amazonasin !== false) {
				...
				if (preg_match("/<td class=\"resultCount\">R.*?sultats \d+ - \d+ sur (\d*).*?(\d*)<\/td>/i", $pageBuffer, $regs) || preg_match("/<td class=\"resultCount\">([0-9]+) r.*?sultats<\/td>/i", $pageBuffer, $regs)) {
	...
	function parse_amazon_game_data($search_attributes_r, $pageBuffer) {
		...
		if (preg_match("/<b>Plate-forme:[\s]*<\/b>([^<]+)<\/li>/si", $pageBuffer, $regs) || preg_match("/<b>Plate-forme:[\s]*<\/b>[^<]+<img [^<]*>([^<]+)<\/li>/si", $pageBuffer, $regs)) {
	...
	function parse_amazon_music_data($search_attributes_r, $pageBuffer) {
		...
		} else if (preg_match("!<div class=\"bucket\">[\s]*<b class=\"h1\">&Eacute;couter des extraits musicaux</b>(.*?)</div>!", $pageBuffer, $regs) || preg_match("!<div class=\"bucket\">[\s]*<b class=\"h1\">Liste des titres</b>(.*?)</div>!", $pageBuffer, $regs)) {
			...
			;
 {
		...
		if (preg_match("/<b>Langue.*?:<\/b>[\s]*([^<]*)<\/li>/i", $pageBuffer, $regs)) {
			...
			$amazon_dvd_audio_map = array(array("Anglais", "2.0"), // Dolby2.0
			...
			$amazon_audio_lang_map = array(array("Anglais"), array("Francais"), array("Espagnol"), array("Italien"), array("Allemand"));
		...
		if (preg_match("/<b>Sous-titre.*?:<\/b>[\s]*([^<]*)<\/li>/i", $pageBuffer, $regs)) {
			...
			$amazon_video_subtitle_map = array(array("Anglais"), array("Francais"), array("Espagnol"), array("Allemand"), array("Italien"));
		...
		if (preg_match("/<li><b>Fonctions DVD[^:]*:<\/b><ul>(.+?)<\/ul>/si", $pageBuffer, $regs)) {
			...
			if (preg_match_all("/<li>(.*)<\/li>/mUi", $dvdFeaturesBlock, $matches)) {
				...
				while (list(, $item) = @each($matches[1])) {
					...
					if (strpos($item, "anamorphic") === FALSE && strpos($item, "Sous-titres") === FALSE && strpos($item, "Langues") === FALSE) {
		...
		} else if (preg_match("/<b>Description<\/b><br[\s]*[\/]*>([^<]*)/si", $pageBuffer, $regs)) {
			$this->addItemAttribute('blurb', $regs[1]);
		}
		//genres:
