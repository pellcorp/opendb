...
function _adodb_getdate($origd = false, $fast = false, $is_gmt = false) {
	...
	return array('seconds' => $secs, 'minutes' => $min, 'hours' => $hour, 'mday' => $day, 'wday' => $dow, 'mon' => $month, 'year' => $year, 'yday' => floor($secsInYear / $_day_power), 'weekday' => gmdate('l', $_day_power * (3 + $dow)), 'month' => gmdate('F', mktime(0, 0, 0, $month, 2, 1971)),
			0 => $origd);
}
...
function adodb_date($fmt, $d = false, $is_gmt = false) {
	...
	for ($i = 0; $i < $max; $i++) {
		...
		// HOUR
		case 'Z':
			$dates .= ($is_gmt) ? 0 : -adodb_get_gmt_different();
			break;
		case 'O':
