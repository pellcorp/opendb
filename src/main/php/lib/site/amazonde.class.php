...
class amazonde extends SitePlugin {
				...
				} else if (preg_match("/ISBN: (\w*)/", strip_tags($pageBuffer), $regs)) // for books, ASIN is the same as ISBN
 {
			...
			if ($amazdeasin !== FALSE) {
				...
				if (preg_match("/<(td|div) class=\"resultCount\">[0-9]+[\s]*-[\s]*[0-9]+ von ([0-9,\.]+) Ergebnissen<\/(td|div)>/i", $pageBuffer, $regs) || preg_match("/<(td|div) class=\"resultCount\">([0-9]+) Ergebnisse<\/(td|div)>/i", $pageBuffer, $regs)
						|| preg_match("/<(td|div) class=\"resultCount\">([0-9]+) Treffer<\/(td|div)>/i", $pageBuffer, $regs) || preg_match("/<h1 id=\"noResultsTitle\".*?(Beste Ergebnisse).*class=\"resultindex\">([0-9]+)/i", $pageBuffer, $regs)) {
					// store total count here.
					$this->setTotalCount($regs[2]);
					//print_r("no_results: ".$regs[2]);

					if (preg_match_all("!<div class=\"productImage\">[\s]*" . "<a href=\"[^\"]+\">[\s]*" . "<img src=\"([^\"]+)\"[^>]*>[\s]*</a>[\s]*</div>[\s]*" . "<div class=\"productData\">[\s]*" . "<div class=\"productTitle\">[\s]*" . "<a href=\"([^\"]+)\">([^<]*)</a>!m", $pageBuffer, $matches)
							|| 
							//books 
							preg_match_all("!<td class=\"imageColumn\"[^>]*>.*?" . "<img.*?src=\"([^\"]+)\"[^>]*>.*?<a href=\"([^\"]+)\"[^>]*><span class=\"srTitle\">([^<]+)</span></a>!m", $pageBuffer, $matches)
							|| 
							// best results
							preg_match_all("!<td height=\"160\" width=\"160\" align=\"center\">.*?" . "<img.*?src=\"([^\"]+)\"[^>]*>.*?<a href=\"([^\"]+)\"[^>]*><span class=\"srTitle\">([^<]+)</span></a>!m", $pageBuffer, $matches)) {
	...
	function queryItem($search_attributes_r, $s_item_type) {
		...
		if (($startIndex = strpos($pageBuffer, "<h2>Anhand des Sachgebietes nach ")) !== FALSE && ($endIndex = strpos($pageBuffer, "Titel/Artikel ALLER gew", $startIndex)) !== FALSE) {
					...
					if (strpos($item, "Ton:") === FALSE) // audio languages already parse this.
 {
		...
		//<li><b>Sprache:</b> Deutsch</li>
		$amazon_dvd_audio_map = array(array("Englisch"), array("Spanisch"), array("Franzosisch"), array("Deutsch"), array("Italienisch"));
