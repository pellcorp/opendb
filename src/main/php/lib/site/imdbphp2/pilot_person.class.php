...
class pilot_person extends person_base {
	...
	protected function set_pagename($wt) {
		switch ($wt) {
		case "Name":
			$urlname = "/people/imdb-id-" . (int) $this->imdbid() . ".json";
			break;
		case "Images":
			if ($this->page["Name"] == "")
				$this->openpage("Name");
	...
	public function photo_array() {
		if (empty($this->photo_array)) {
			if ($this->page["Images"] == "")
				$this->openpage("Images");
			$icount = count($this->page["Images"]);
			if (!$icount)
				return array();
			foreach ($this->page["Images"] as $img) {
				$this->photo_array[] = array("width" => $img->width, "size" => $img->size, "url" => $img->base_url . $img->photo_id . '/' . $img->file_name_base . '.' . $img->extension);
			}
		}
	...
	public function movies_all() {
		if (empty($this->allfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->allfilms = $this->imdb->movies_all();
	...
	public function movies_actress() {
		if (empty($this->actressfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->actressfilms = $this->imdb->movies_actress();
	...
	public function movies_actor() {
		if (empty($this->actorsfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->actorsfilms = $this->imdb->movies_actor();
	...
	public function movies_producer() {
		if (empty($this->producersfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->producersfilms = $this->imdb->movies_producer();
	...
	public function movies_director() {
		if (empty($this->directorsfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->directorsfilms = $this->imdb->movies_director();
	...
	public function movies_soundtrack() {
		if (empty($this->soundtrackfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->soundtrackfilms = $this->imdb->movies_soundtrack();
	...
	public function movies_crew() {
		if (empty($this->crewsfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->crewsfilms = $this->imdb->movies_crew();
	...
	public function movies_thanx() {
		if (empty($this->thanxfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->thanxfilms = $this->imdb->movies_thanx();
	...
	public function movies_self() {
		if (empty($this->selffilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->selffilms = $this->imdb->movies_self();
	...
	public function movies_writer() {
		if (empty($this->writerfilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->writerfilms = $this->imdb->movies_writer();
	...
	public function movies_archive() {
		if (empty($this->archivefilms) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->archivefilms = $this->imdb->movies_archive();
	...
	public function birthname() {
		if (empty($this->birth_name) && $this->pilot_imdbfill > BASIC_ACCESS)
			$this->birth_name = $this->imdb->birthname();
	...
	public function nickname() {
		if (empty($this->nick_name) && $this->pilot_imdbfill > BASIC_ACCESS)
			$this->nick_name = $this->imdb->nickname();
	...
	public function height() {
		if (empty($this->bodyheight) && $this->pilot_imdbfill > BASIC_ACCESS)
			$this->bodyheight = $this->imdb->height();
	...
	public function spouse() {
		if (empty($this->spouses) && $this->pilot_imdbfill > BASIC_ACCESS)
			$this->spouses = $this->imdb->spouse();
		return $this->spouses;
	}
	...
	public function bio() {
		if (empty($this->bio_bio)) {
			if ($this->pilot_imdbfill == FULL_ACCESS)
				$this->bio_bio = $this->imdb->bio();
			else {
				if ($this->page["Name"] == "")
					$this->openpage("Name");
	...
	public function trivia() {
		if (empty($this->bio_trivia) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->bio_trivia = $this->imdb->trivia();
	...
	public function quotes() {
		if (empty($this->bio_quotes) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->bio_quotes = $this->imdb->quotes();
	...
	public function trademark() {
		if (empty($this->bio_tm) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->bio_tm = $this->imdb->trademark();
		return $this->bio_tm;
	}
	...
	public function salary() {
		if (empty($this->bio_salary) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->bio_salary = $this->imdb->salary();
	...
	public function pubprints() {
		if (empty($this->pub_prints) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_prints = $this->imdb->pubprints();
	...
	public function pubmovies() {
		if (empty($this->pub_movies) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_movies = $this->imdb->pubmovies();
	...
	public function pubportraits() {
		if (empty($this->pub_portraits) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_portraits = $this->imdb->pubportraits();
	...
	public function interviews() {
		if (empty($this->pub_interviews) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_interviews = $this->imdb->interviews();
	...
	public function articles() {
		if (empty($this->pub_articles) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_articles = $this->imdb->articles();
	...
	public function pictorials() {
		if (empty($this->pub_pictorials) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_pictorials = $this->imdb->pictorials();
	...
	public function magcovers() {
		if (empty($this->pub_magcovers) && $this->pilot_imdbfill == FULL_ACCESS)
			$this->pub_magcovers = $this->imdb->magcovers();
