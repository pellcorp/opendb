		...
		if(strlen($pageBuffer)>0)
		{
			...
			if(preg_match_all('!<b><a href=[\'|"]/Exec/v4_item.asp\?userid=[0-9]*&amp;item_id=([0-9]+)&amp;searchID=[0-9]+[^\'|^"]*[\'|"]>(.*?)</a>[\s]*</b>*!mi', $pageBuffer, $matches))
			{
				for ($i = 0; $i < count($matches[1]); $i++)
				{
					...
					if(preg_match('!<img src=[\'|"](http://images2\.dvdempire\.com/gen/movies/'.$matches[1][$i].'t.jpg)[\'|"]!', $pageBuffer, $regs))
						$thumbimg = $regs[1];
					else
						$thumbimg = NULL;
...
//					else
	...
	{	
		...
		if(preg_match('!<img src=[\'|"](http://images2\.dvdempire\.com/gen/movies/'.$search_attributes_r['dvdempr_id'].'\.jpg)[\'|"]!', $buffer, $regs))
		{
		...
		if(strlen($buffer)>0)
		{
			...
			if(preg_match('!<img src=[\'|"](http://images2\.dvdempire\.com/gen/movies/'.$search_attributes_r['dvdempr_id'].'h\.jpg)[\'|"]!', $buffer, $regs))
			{
		...
		if(strlen($buffer)>0)
		{
			if(preg_match('!<img src=[\'|"](http://images2\.dvdempire\.com/gen/movies/'.$search_attributes_r['dvdempr_id'].'bh\.jpg)[\'|"]!', $buffer, $regs))
			{
