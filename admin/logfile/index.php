...
if (strlen($logging_config_r['file']) > 0) {
	if (is_readable($logging_config_r['file'])) {
		...
		if ($HTTP_VARS['op'] == '') {
			...
			echo ("<p>[<a href=\"$PHP_SELF?type=$ADMIN_TYPE&op=clear\">" . get_opendb_lang_var('clear_usagelog') . "</a>] " . "[<a href=\"$PHP_SELF?type=$ADMIN_TYPE&op=backup\">" . get_opendb_lang_var('backup_usagelog') . "</a>] " . "[<a href=\"$PHP_SELF?type=$ADMIN_TYPE&op=download&mode=job\">"
					. get_opendb_lang_var('download_usagelog') . "</a>]</p>");
			...
			if ($logfile) {
				...
				echo ("\n<tr class=\"navbar\">" . "<th>" . get_opendb_lang_var('log_type') . "</th>" . "<th>" . get_opendb_lang_var('user') . "</th>" . "<th>" . get_opendb_lang_var('user_ip') . "</th>" . "<th>" . get_opendb_lang_var('date') . "</th>" . "<th>" . get_opendb_lang_var('file') . "</th>"
						. "<th>" . get_opendb_lang_var('function') . "</th>" . "<th>" . get_opendb_lang_var('message') . "</th>" . "<th>" . get_opendb_lang_var('parameters') . "</th>" . "</tr>");
				...
				while (($tokens = fget_tokenised_log_entry($logfile)) !== FALSE) {
					...
					echo ("\n<tr class=\"logRow\">" . "<td class=\"$class\">" . $tokens['type'] . "</td>" . "<td class=\"$class\">" . $tokens['user_id'] . "</td>" . "<td class=\"$class\">" . $tokens['ip'] . "</td>" . "<td class=\"$class dateTime\">" . $tokens['datetime'] . "</td>"
							. "<td class=\"$class\">" . $tokens['file'] . "</td>" . "<td class=\"$class\">" . $tokens['function'] . "</td>" . "<td class=\"$class\">" . $tokens['message'] . "</td>" . "<td class=\"$class\">" . $tokens['parameters'] . "</td>" . "</tr>");
				}
				fclose($logfile);
