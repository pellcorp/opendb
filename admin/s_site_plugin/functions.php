...
function fetch_max_site_plugin_order_no() {
	$query = "SELECT MAX(order_no) as max_order_no " . "FROM s_site_plugin";
...
function fetch_site_item_attribute_type_rs($site_type, $s_item_type = NULL) {
	$query = "SELECT siat.s_item_type, siat.s_attribute_type, siat.order_no, siat.prompt, siat.compulsory_ind " . "FROM s_item_attribute_type siat, s_attribute_type sat " . "WHERE sat.s_attribute_type = siat.s_attribute_type AND sat.site_type = '$site_type' ";
...
function fetch_lookup_s_attribute_type_rs() {
	$query = "SELECT DISTINCT sat.s_attribute_type " . "FROM s_attribute_type_lookup satl, " . "s_attribute_type sat " . "WHERE sat.s_attribute_type = satl.s_attribute_type " . "ORDER BY satl.s_attribute_type";
...
function insert_s_site_plugin($site_type, $classname, $order_no, $title, $image, $description, $external_url, $items_per_page, $more_info_url) {
	if (strlen($site_type) > 0) {
		...
		$query = "INSERT INTO s_site_plugin (site_type, classname, order_no, title, image, description, external_url, items_per_page, more_info_url) " . "VALUES ('$site_type', '$classname', $order_no, '$title', '$image', '$description', '$external_url', $items_per_page, '$more_info_url')";
...
function update_s_site_plugin($site_type, $classname, $order_no, $title, $image, $description, $external_url, $items_per_page, $more_info_url) {
	if (strlen($site_type) > 0) {
		...
		$query = "UPDATE s_site_plugin " . "SET description = " . ($description !== FALSE ? "'" . addslashes(trim(strip_tags($description))) . "'" : "description") . ($classname !== FALSE ? ", classname = '" . $classname . "'" : "") . ($order_no !== FALSE ? ", order_no = $order_no" : "")
				. ($title !== FALSE ? ", title = '" . addslashes(trim(strip_tags($title))) . "'" : "") . ($image !== FALSE ? ", image = '" . addslashes(trim(strip_tags($image))) . "'" : "") . ($external_url !== FALSE ? ", external_url = '" . addslashes(trim(strip_tags($external_url))) . "'" : "")
				. ($items_per_page !== FALSE ? ", items_per_page = $items_per_page" : "") . ($more_info_url !== FALSE ? ", more_info_url = '" . addslashes(trim(strip_tags($more_info_url))) . "'" : "") . " WHERE site_type = '$site_type'";
...
function insert_s_site_plugin_conf($site_type, $name, $keyid, $value, $description = NULL) {
	if (strlen($site_type) > 0 && strlen($name) > 0) {
		...
		$query = "INSERT INTO s_site_plugin_conf (site_type, description, name, keyid, value) " . "VALUES ('$site_type', '$description', '$name', '$keyid', '$value')";
		$insert = db_query($query);
...
function update_s_site_plugin_conf($site_type, $name, $keyid, $value, $description = FALSE) {
	if (strlen($site_type) > 0 && strlen($name) > 0 && strlen($keyid) > 0) {
		...
		$query = "UPDATE s_site_plugin_conf " . "SET description = " . ($description !== FALSE ? "'" . addslashes(trim(strip_tags($description))) . "'" : "description") . ($value !== FALSE ? ", value = '" . addslashes(trim(strip_tags($value))) . "'" : "")
...
function insert_s_site_plugin_input_field($site_type, $field, $order_no, $description, $prompt, $field_type, $default_value, $refresh_mask) {
	if (strlen($site_type) > 0 && strlen($field) > 0) {
		...
		$query = "INSERT INTO s_site_plugin_input_field (site_type, field, order_no, description, prompt, field_type, default_value, refresh_mask) " . "VALUES ('$site_type', '$field', $order_no, '$description', '$prompt', '$field_type', '$default_value', '$refresh_mask')";
...
function update_s_site_plugin_input_field($site_type, $field, $order_no, $description, $prompt, $field_type, $default_value, $refresh_mask) {
	if (strlen($site_type) > 0 && strlen($field) > 0) {
		...
		$query = "UPDATE s_site_plugin_input_field " . "SET description = " . ($description !== FALSE ? "'" . addslashes(trim(strip_tags($description))) . "'" : "description") . ($order_no !== FALSE ? ", order_no = " . $order_no . "" : "")
				. ($prompt !== FALSE ? ", prompt = '" . addslashes(trim(strip_tags($prompt))) . "'" : "") . ($field_type !== FALSE ? ", field_type = '" . $field_type . "'" : "") . ($default_value !== FALSE ? ", default_value = '" . addslashes(trim(strip_tags($default_value))) . "'" : "")
				. ($refresh_mask !== FALSE ? ", refresh_mask = '" . addslashes(trim(strip_tags($refresh_mask))) . "'" : "") . " WHERE site_type = '$site_type' AND field = '$field'";
...
function insert_s_site_plugin_s_attribute_type_map($site_type, $variable, $s_item_type_group, $s_item_type, $s_attribute_type, $lookup_attribute_val_restrict_ind) {
	if (strlen($site_type) > 0 && strlen($variable) > 0 && strlen($s_item_type_group) > 0 && strlen($s_item_type) > 0 && strlen($s_attribute_type) > 0) {
		...
		if ($s_item_type_group !== FALSE && $s_item_type !== FALSE && strlen($s_item_type) > 0 && strlen($s_item_type_group) > 0 && 
...
function update_s_site_plugin_s_attribute_type_map($site_type, $sequence_number, $s_item_type_group, $s_item_type, $s_attribute_type, $lookup_attribute_val_restrict_ind) {
	if (strlen($s_item_type_group) > 0 && is_numeric($sequence_number)) {
		...
		$query = "UPDATE s_site_plugin_s_attribute_type_map " . "SET variable = variable " . ($s_item_type_group !== FALSE ? ", s_item_type_group = '" . $s_item_type_group . "'" : "") . ($s_item_type !== FALSE ? ", s_item_type = '" . $s_item_type . "'" : "")
				. ($s_attribute_type !== FALSE ? ", s_attribute_type = '" . $s_attribute_type . "'" : "") . ($lookup_attribute_val_restrict_ind !== FALSE ? ", lookup_attribute_val_restrict_ind = '" . $lookup_attribute_val_restrict_ind . "'" : "")
...
function delete_s_site_plugin_s_attribute_type_map($site_type, $sequence_number = NULL) {
	if (strlen($site_type) > 0) {
		...
		$query = "DELETE FROM s_site_plugin_s_attribute_type_map " . "WHERE site_type = '$site_type'";
...
function insert_s_site_plugin_s_attribute_type_lookup_map($site_type, $s_attribute_type, $value, $lookup_attribute_val) {
	if (strlen($site_type) > 0 && strlen($s_attribute_type) > 0 && strlen($value) > 0 && strlen($lookup_attribute_val) > 0) {
		...
		$query = "INSERT INTO s_site_plugin_s_attribute_type_lookup_map (site_type, s_attribute_type, value, lookup_attribute_val) " . "VALUES ('$site_type', '$s_attribute_type', '$value', '$lookup_attribute_val')";
...
function update_s_site_plugin_s_attribute_type_lookup_map($site_type, $sequence_number, $lookup_attribute_val) {
	if (strlen($site_type) > 0 && is_numeric($sequence_number)) {
		...
		$query = "UPDATE s_site_plugin_s_attribute_type_lookup_map " . "SET lookup_attribute_val = '" . addslashes(trim(strip_tags($lookup_attribute_val))) . "'" . " WHERE site_type = '$site_type' AND sequence_number = $sequence_number";
...
function delete_s_site_plugin_s_attribute_type_lookup_map($site_type, $sequence_number = NULL) {
	if (strlen($site_type) > 0) {
		...
		$query = "DELETE FROM s_site_plugin_s_attribute_type_lookup_map " . "WHERE site_type = '$site_type'";
...
function insert_s_site_plugin_link($site_type, $s_item_type_group, $s_item_type, $order_no, $description, $url, $title_url) {
	if (strlen($site_type) > 0 && strlen($s_item_type_group) > 0 && strlen($s_item_type) > 0 && is_numeric($order_no)) {
		...
		// make sure only one of s_item_type_group and s_item_type is configured.
		if (strlen($s_item_type) > 0 && strlen($s_item_type_group) > 0 && 
				// if both are set to '*', its not a problem.
				($s_item_type != '*' || $s_item_type_group != '*')) {
			if ($s_item_type != '*')
				$s_item_type_group = '*';
		...
		$query = "INSERT INTO s_site_plugin_link (site_type, s_item_type_group, s_item_type, order_no, description, url, title_url) " . "VALUES ('$site_type', '$s_item_type_group', '$s_item_type', $order_no, '$description', '$url', '$title_url')";
...
function update_s_site_plugin_link($site_type, $sequence_number, $s_item_type_group, $s_item_type, $order_no, $description, $url, $title_url) {
	if (strlen($site_type) > 0 && is_numeric($sequence_number)) {
		...
		$query = "UPDATE s_site_plugin_link " . "SET description = " . ($description !== FALSE ? "'" . addslashes(trim(strip_tags($description))) . "'" : "description") . ($url !== FALSE ? ", url = '" . addslashes(trim(strip_tags($url))) . "'" : "")
				. ($title_url !== FALSE ? ", title_url = '" . addslashes(trim(strip_tags($title_url))) . "'" : "") . ($order_no !== FALSE && is_numeric($order_no) ? ", order_no = $order_no" : "") . ($s_item_type_group !== FALSE ? ", s_item_type_group = '$s_item_type_group'" : "")
