...
class SitePlugin
{
	...
	*/
	function getItemData($s_item_type = NULL)
	{
		if(is_array($this->_item_data_r))
		{
			...
			{
				return get_expanded_and_mapped_site_plugin_item_variables_r(
																$this->_type,
																$s_item_type,
																$this->_item_data_r);
			}
	...
	*/
	...
	function setError($error, $details = NULL)
	{
		if(strlen($error)>0)
		{
			$this->_errors[] = array('error'=>$error,'detail'=>$details);
		}
	}
	...
	*/
	function addItemAttribute($attribute, $value, $options = NULL, $keyname = NULL)
	{
		...
		else
		{
			...
			$replace = array(utf8_encode(chr(160)) => ' '); //thawn: fixes issue with 3param version of strtr() not being utf-8 compatible.
			...
			if(strlen($value)>0)
			{
				...
				else // otherwise single value only
				{
					...
					{
						$this->_item_data_r[$attribute] = array($keyname => $value);
					}
				}
			}//if(strlen($value)>0)
		}
