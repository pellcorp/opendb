...
class imdb extends movie_base {
	...
	protected function set_pagename($wt) {
		...
		case "Title":
			...
			break;
		case "TitleFoot":
			$urlname = "/_ajax/iframe?component=footer";
			break;
		case "Credits":
			$urlname = "/fullcredits";
			break;
	...
	private function title_year() {
		...
		if (@preg_match('!<title>(IMDb\s*-\s*)?(.*) \((.*)(\d{4}|\?{4}).*\)(.*)(\s*-\s*IMDb)?</title>!', $this->page["Title"], $match)) {
			...
			if (empty($match[3]))
				$main_movietype = 'Movie';
			else
				$main_movietype = $match[3];
			if ($match[3] == "????")
				$this->main_year = "";
			else
	...
	public function get_episode_details() {
		...
		if ($this->page["Title"] == "")
			$this->openpage("Title");
		$preg = '!<h2 class="tv_header">\s*<a\s+href="/title/tt(?<seriesimdbid>\d{7})/"\s*>(?<seriestitle>.+?)</a>:\s*' . '<span class="nobr">\s*Season\s+(?<season>\d+),\s+Episode\s+(?<episode>\d+)\s*</span>\s*' . '</h2>\s*<h1 class="header" itemprop="name">\s*'
				. '(?<episodetitle>.+)\s*<span class="nobr">\((?<airdate>.+?)\)</span>!';
		if (preg_match($preg, $this->page["Title"], $match)) {
			$info = array("imdbid" => $match['seriesimdbid'], "seriestitle" => $match['seriestitle'], "series_prodtime" => '', "episodetitle" => $match['episodetitle'], "season" => $match['season'], "episode" => $match['episode'], "airdate" => $match['airdate']);
			return $info;
		} else
		...
		if (empty($this->akas)) {
			...
			for ($i = 0; $i < count($matches[1]); $i += 2) {
				...
				foreach ($countries as $country) {
					...
					$this->akas[] = array("title" => $title, "year" => '', "country" => $_country, "comment" => $comment, "lang" => '');
				}
			}
		}
		return $this->akas;
			...
			if (preg_match('!<select id="bySeason"(.*?)</select!ims', $this->page["Episodes"], $match)) {
				...
				for ($i = 0; $i < count($matches[0]); ++$i) {
					...
					if ($this->page["Episodes-$s"] == "cannot open page")
						continue;
					// no such page
					$preg = '!<div class="info" itemprop="episodes".+?>\s*<meta itemprop="episodeNumber" content="(?<episodeNumber>\d+)"/>\s*' . '<div class="airdate">\s*(?<airdate>.+?)\s*</div>\s*' . '.+?\shref="/title/tt(?<imdbid>\d{7})/"\s+title="(?<title>.+?)"\s+itemprop="name"'
		...
		if (empty($this->goofs)) {
			...
			if (@preg_match_all('@<h4 class="li_group">(.+?)(!?&nbsp;)</h4>\s*(.+?)\s*(<a name="|<div id="top_rhs_wrapper")@ims', $this->page["Goofs"], $matches)) {
				...
				for ($i = 0; $i < $gc; ++$i) {
					...
					$ic = count($goofy[0]);
					for ($k = 0; $k < $ic; ++$k)
						$this->goofs[] = array("type" => $matches[1][$i], "content" => str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $goofy[1][$k]));
				}
			}
		}
		return $this->goofs;
	...
	public function trailers($full = FALSE, $all = TRUE) {
		if (empty($this->trailers)) {
			...
			if ($has_trailers !== FALSE) { // if any on-site or off-site trailers exists
				...
				if ($all)
					$regex = '@<a\s*onclick=".*?"\s*href="(/video/.*?/vi\d+/)".*?><img.*?title="(.*?)"\s*src="(.*?)"@s';
				else
		...
		if (empty($this->soundtracks)) {
			...
			if (preg_match('|<p class="disclaimer">(.*?)</script|ims', $this->page["Soundtrack"], $match)) {
				if (preg_match_all("/<li>(.*?)<\/b><br>(.*?)<br>(.*?)<br>.*?<\/li>/", str_replace("\n", " ", $match[1]), $matches)) {
					$mc = count($matches[0]);
					for ($i = 0; $i < $mc; ++$i)
						$this->soundtracks[] = array("soundtrack" => $matches[1][$i], "credits" => array(str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $matches[2][$i]), str_replace('href="/', 'href="http://' . $this->imdbsite . '/', $matches[3][$i])));
				}
			}
		}
	...
	public function awards() {
		if (empty($this->awards)) {
			...
			for ($i = 0; $i < $rowcount; $i++) {
				...
				if (count($cels) == 0) {
					$this->awards[$festival]['entries'][$nr] = array('year' => $year, 'won' => $won, 'category' => $category, 'award' => $award, 'people' => $people, 'comment' => $comment);
				}
			}
		}
		return $this->awards;
